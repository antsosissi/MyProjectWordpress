<?php
/**
 * classe de service pour le type de post equipe
 *
 * @package WordPress
 * @subpackage bondy
 * @since bondy 1.0
...
class CEquipe {
	...
	private static function _load($pid) {
		...
		if ( $p->post_type == POST_TYPE_EQUIPE ) {
			...
			if ( class_exists('Acf') ) {
				//Photo équipe
				$element->photo_equipe_id = get_field(FIELD_EQUIPE_PHOTO, $p->ID);

				//Nom équipe
				$element->nom_equipe = get_field(FIELD_EQUIPE_NOM, $p->ID);

				//Prénom équipe
				$element->prenom_equipe = get_field(FIELD_EQUIPE_PRENOM, $p->ID);

				//Fonction équipe
				$element->fonction_equipe = get_field(FIELD_EQUIPE_FONCTION, $p->ID);

				//Citation équipe
				$element->citation_equipe = get_field(FIELD_EQUIPE_CITATION, $p->ID);

				//facebook équipe
				$element->equipe_facebook = get_field(FIELD_EQUIPE_FACEBOOK, $p->ID);
				//instagram équipe
				$element->equipe_instagram = get_field(FIELD_EQUIPE_INSTAGRAM, $p->ID);
				//linkedin équipe
				$element->equipe_linkedin = get_field(FIELD_EQUIPE_LINKEDIN, $p->ID);
			}
	...
	public static function manage_column($column_name, $post_id){
		$equipe = self::getById($post_id);
		switch($column_name){
			case 'vignette':
                list($vignette)  = wp_get_attachment_image_src($equipe->photo_equipe_id, IMAGE_SIZE_ACTUS_VIGNETTE);
...
add_action( 'manage_edit-' . POST_TYPE_EQUIPE . '_columns' , 'CEquipe::add_column' );
add_action( 'manage_' . POST_TYPE_EQUIPE . '_posts_custom_column', 'CEquipe::manage_column', 10, 2 );
