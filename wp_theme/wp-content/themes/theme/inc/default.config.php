...
//enlever le p auto et br auto
add_filter('tiny_mce_before_init','theme_tiny_mce_before_init',10,2);
function theme_tiny_mce_before_init($mceInit, $editor_id){
...
function theme_default_scripts(){
  ...
  if(!is_admin()){
    ...
    wp_register_script('jquery', get_template_directory_uri() .'/assets/js/library/jquery-1.9.min.js', false, '1.9.0',false);
    wp_enqueue_script('jquery');
  }

  wp_enqueue_script('jquery-lazyload', get_template_directory_uri() .'/assets/js/library/jquery.lazyload.min.js', array('jquery'), '1.8.0' , true);
}

//add_filter( 'the_content','theme_default_the_content',9);
function theme_default_the_content($html){
